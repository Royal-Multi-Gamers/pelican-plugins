...
class PlayerCounterPluginProvider extends ServiceProvider
{
    ...
    {
        // Register the local GameQ autoloader
        $this->registerGameQAutoloader();
    ...
    public function boot(): void
    {
        ...
        // Run the seeder to link eggs to queries on installation
        $this->linkEggsToQueries();
    }
    
    /**
     * Automatically link eggs to queries on installation
     */
    protected function linkEggsToQueries(): void
    {
        // Only run if there are eggs but no egg-query links yet
        if (Egg::count() > 0 && EggGameQuery::count() === 0) {
    ...
    /**
     * Register the local GameQ autoloader to use the bundled GameQ library
     * instead of requiring it via Composer.
     */
    protected function registerGameQAutoloader(): void
    {
        $autoloaderPath = __DIR__ . '/../GameQ/Autoloader.php';
